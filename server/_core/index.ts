...
import { saveN8nWebhookData } from "../db";
...
async function startServer() {
  ...
  app.post("/webhook/n8n", async (req, res) => {
    try {
      ...
      await saveN8nWebhookData(dataType, data);
      
      res.json({ success: true, message: `Webhook data received for ${dataType}` });
    } catch (error) {
      console.error("[Webhook] Error processing n8n webhook:", error);
