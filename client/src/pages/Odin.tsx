...
import { useGoogleSheets, parseOdin } from "@/hooks/useGoogleSheets";
...
export default function Odin() {
  const { data: sheetsData, loading, error } = useGoogleSheets("Odin");
  const [sortKey, setSortKey] = useState<SortKey>("route");
  const [sortDir, setSortDir] = useState<SortDir>("asc");
  const [isRefreshing, setIsRefreshing] = useState(false);

  const routes = parseOdin(sheetsData);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4">
        <div className="text-center">
          ...
          <p className="text-sm text-gray-600">Indlæser ODIN-data...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen pb-24" style={{ background: "oklch(0.99 0.001 286)" }}>
      ...
      <div className="sticky top-0 z-10 p-4 sm:p-6" style={{ background: "linear-gradient(135deg, oklch(0.15 0.01 286) 0%, oklch(0.25 0.01 286) 100%)" }}>
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <div>
            <h1 className="text-lg sm:text-2xl font-bold text-white">ODIN</h1>
            <p className="text-xs sm:text-sm text-white/70 mt-1">Rute Performance</p>
          </div>
          <button
